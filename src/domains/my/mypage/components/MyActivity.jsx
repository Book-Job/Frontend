    ...
    <div className='pt-5 sm:pt-20 sm:px-16 px-7 '>
      ...
      <div className='flex flex-col gap-12 sm:flex-row justify-evenly '>
        {activities.map((activity, idex) => (
          ...
          >
            <div className='bg-[#FDF8FA] sm:w-[100px] sm:h-[100px] w-[70px] h-[70px] rounded-full items-center flex justify-center hover:animate-bounce transition'>
              <img src={activity.icon} alt={activity.alt} className='object-contain w-1/2 h-1/2' />
            </div>
            <p className='text-lg font-medium transition sm:text-2xl hover:scale-110'>{activity.text}</p>
          </button>
        ))}
      </div>
    </div>
